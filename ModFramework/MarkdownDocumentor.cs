...
{
    private Dictionary<Type, List<object>> data = new();

    public delegate string TypeFormatter<TRecord>(bool header, TRecord? record);

    private static Dictionary<Type, TypeFormatter<object>> _typeFormatters = new();
    ...
    public void Write(string filename)
    {
        ...
        foreach (var pair in data)
        {
            ...
            // header
            var line = formatter?.Invoke(true, null);
            if (line is not null && CanWrite(true, ref line))
                writer.WriteLine(line);

            // data
            foreach (var item in pair.Value)
            {
                line = formatter?.Invoke(false, item);
                if (line is not null && CanWrite(false, ref line))
                    writer.WriteLine(line);
            }
